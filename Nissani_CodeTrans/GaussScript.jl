...
ker1 = MvNormal([-0.5;-0.5], Diagonal(ones(2)))
ker2 = MvNormal([6.5; 6.5], Diagonal(ones(2)))
ker3 = MvNormal([-2.5, 4.5], Diagonal([2.; 2.]))
#Create the kernels to be evaluated
x1 = rand(ker1, 1000)
x2 = rand(ker2, 1000)
x3 = rand(ker3, 1000)
#Shuffle the kernels
cv_X = shuffleobs(hcat(x1, x2, x3))
#Initialize hyperparameters
epoch_nr = 100;
nmr_training_batches = 1
test_batch_size = 10
train_batch_size = 3000
classes = ["Ker1", "Ker2", "Ker3"]
d = 2
#Train the model
(w_N, wx_N, θ, μ₁, μ₂, c1, c2, y_N) = MultiEpoch(hcat(x1, x2, x3), nmr_training_batches, d, train_batch_size, epoch_nr)
#Get equations of lines defining the hyperplanes
fs1(x) = (w_N[2,1]/(w_N[1,1] - θ[1]))*x + (w_N[1,2] - (w_N[2,1]/(w_N[1,1] - θ[1]))*w_N[1,1]);
fs2(x) = (w_N[2,2]/(w_N[1,2] - θ[2]))*x + (w_N[2,2] - (w_N[2,2]/(w_N[1,2] - θ[2]))*w_N[1,2]);
fs3(x) = (w_N[2,3]/(w_N[1,3] - θ[3]))*x + (w_N[1,3] - (w_N[2,3]/(w_N[1,3] - θ[3]))*w_N[1,3]);
